...
export class MigrationRemovedFileResolve implements Resolve<IMigrationRemovedFile> {
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<IMigrationRemovedFile> {
        const id = route.params['id'] ? route.params['id'] : null;
        if (id) {
            return this.service.find(id).pipe(
                filter((response: HttpResponse<MigrationRemovedFile>) => response.ok),
                map((migrationRemovedFile: HttpResponse<MigrationRemovedFile>) => migrationRemovedFile.body)
            );
        }
